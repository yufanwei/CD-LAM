...
Create or verify the isolated Linux x86-64 CUDA 12.8 environment used by the
CD-LAM 2B runtime. The command fetches pinned source and Python packages only;
it never downloads model weights, datasets, tokenizer files, or text encoders.

Options:
  --environment PATH  Target environment (default: .deps/model-env).
...
MODEL_ENV="$(absolute_path "${MODEL_ENV:-$DEPS_ROOT/model-env}")"
...
if [[ "$DRY_RUN" == yes ]]; then
  ...
  printf 'DRY RUN: create isolated environment %q with CPython 3.10\n' "$MODEL_ENV"
...
if [[ -e "$MODEL_ENV" ]]; then
  ...
  echo "bootstrap_model_runtime: existing environment verified; no files changed"
  exit 0
fi
...
CD-LAM 2B model environment is ready.
  Python:   $MODEL_ENV/bin/python
  torchrun: $MODEL_ENV/bin/torchrun
  source:   $ACWM_ROOT

No model, tokenizer, text-encoder, dataset, or metric asset was downloaded.
Set paths.python and paths.torchrun in configs/runtime.json to the executables
above, add your licensed local assets, then run:
