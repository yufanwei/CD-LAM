...
Commands:
  ...
  train-smoke        Run the lightweight four-stage optimizer smoke.
  ...
  check              Run all source-release gates.
...
if [[ ! -x "$PY" ]]; then
  echo "CD-LAM GPU environment is missing: $PY" >&2
  echo "Run: bash setup.sh --accept-base-license" >&2
  exit 2
fi
