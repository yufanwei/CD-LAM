...
def test_runtime_overlay_manifest_is_safe_and_complete() -> None:
    ...
    assert {
        ...
        "cdlam_integration/bridge/encoder.py",
        "cdlam_integration/bridge/train.py",
        "cdlam_integration/lam/train.py",
        "cdlam_integration/stage3/train.py",
        "cdlam_integration/world_model/monitor.py",
        "cdlam_integration/world_model/train.py",
    } <= path_strings
    assert not any("training_scope/LAM" in path for path in path_strings)
    legacy_pipeline = "lamwm_" + "pipline"
    legacy_stage_root = "New " + "LAM"
    assert not any(legacy_pipeline in path for path in path_strings)
    assert not any(legacy_stage_root in path for path in path_strings)
