...
def test_profile_preserves_model_environment_python_symlink(tmp_path: Path) -> None:
    target = tmp_path / "base-python"
    target.write_text("fixture", encoding="utf-8")
    model_python = tmp_path / "model-env" / "bin" / "python"
    model_python.parent.mkdir(parents=True)
    model_python.symlink_to(target)
    profile = _profile(tmp_path, tmp_path / "acwm")
    document = json.loads(profile.read_text(encoding="utf-8"))
    document["paths"]["python"] = "model-env/bin/python"
    profile.write_text(json.dumps(document), encoding="utf-8")

    config = RuntimeConfig.load(profile)

    assert config.path("python") == model_python
