...
from cd_lam.__main__ import _build_parser, main


ROOT = Path(__file__).resolve().parents[1]


def test_public_cli_has_no_host_only_training_mode() -> None:
    parser = _build_parser()
    parsed = parser.parse_args(["train-smoke", "--steps", "1"])
    assert parsed.command == "train-smoke"
    with pytest.raises(SystemExit):
        parser.parse_args(["train-smoke", "--device", "cpu"])
