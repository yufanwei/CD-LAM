...
def test_ordered_synthetic_pipeline_trains_and_chains_real_artifacts(tmp_path) -> None:
    ...
    for metadata in (stage2_metadata, bridge_metadata, stage3_metadata):
        ...
        assert metadata["adapter_identity"] == "cd_lam.synthetic_cuda"


def test_ordered_integration_pipeline_runs_on_cuda(tmp_path) -> None:
    output_root = tmp_path / "cuda-smoke"
    config = PipelineConfig.synthetic(output_root, seed=7, device="cuda")
    results, summary_path = run_synthetic_pipeline(
        config,
    ...
    assert [result.adapter_identity for result in results] == [
        "cd_lam.synthetic_cuda"
    ] * 4
    assert all(result.checkpoint.is_file() for result in results)
