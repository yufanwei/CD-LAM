...
def test_delta_of_minmax_normalized_actions_has_expected_algebra() -> None:
    actions = torch.tensor(
        [[[1.0, -1.0], [3.0, 2.0], [7.0, 5.0], [9.0, 8.0]]]
    )
    minimum = torch.tensor([-1.0, -4.0])
    maximum = torch.tensor([9.0, 8.0])
    stride = 2
    actual = minmax_normalized_deltas(
        actions, minimum, maximum, stride=stride
    )
    expected = (
        2.0
        * strided_deltas(actions, stride=stride)
        / (maximum - minimum)
    )
...
def test_normalized_block_anchor_conversion_recovers_raw_adjacent_actions() -> None:
    ...
    normalized_cumulative = adjacent_to_block_anchor(
        normalized_adjacent, block_size=4
    )
