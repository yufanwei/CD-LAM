    ...
    *,
    ...
    for identity, kind, checkpoint_format, relative, step, role in (
        ...
        (
            ...
            "models/posttrain/model.pt",
            3000,
            "unverified",
        ),
    ):
        value = content if identity == "posttrain" else identity.encode()
        asset = root / relative
        ...
        assets.append(
            {
                ...
                "model_scale": "2B",
                "data_tier": "posttrain" if identity == "posttrain" else "multi-source",
                "optimizer_step": step,
                "paper_role": role,
                "path": relative,
                "bytes": len(value),
    ...
    bridge = root / "models" / "posttrain" / "bridge.pt"
    bridge.write_bytes(bridge_content)
    contract_content = b'{"contract_id":"test"}\n'
    contract = root / "models" / "posttrain" / "action_contract.json"
        ...
        "auxiliary_files": [
            {
                "id": "posttrain-bridge",
                "kind": "bridge",
                "main_model_id": "posttrain",
                "path": "models/posttrain/bridge.pt",
            ...
            {
                "id": "posttrain-action-contract",
                "kind": "json_contract",
                "main_model_id": "posttrain",
                "path": "models/posttrain/action_contract.json",
    ...
    return root / "models" / "posttrain" / "model.pt"


def test_selective_download_always_requests_asset_manifest() -> None:
    assert DOWNLOAD_MODELS.effective_allow_patterns(["models/posttrain/*"]) == [
        "models/posttrain/*",
        "asset_manifest.json",
    ]
...
def test_posttrain_selection_verifies_model_and_auxiliary_files(tmp_path) -> None:
    ...
    assert (
        DOWNLOAD_MODELS.validate_downloaded_snapshot(
            tmp_path, ["models/posttrain/model.pt"]
        )
        == expected
    )

    contract = tmp_path / "models/posttrain/action_contract.json"
    contract.unlink()
    with pytest.raises(
