...
Shard schema (datasets/data_recipe_v1_7/<source>/shard-*.parquet):
...
_NEEDED_COLS = (
    "episode_id", "task_id", "num_frames", "video_mp4",
    "timestamp", "step_starts", "step_ends", "step_actions", "step_skills",
)
...
# ============================================================================
def _center_crop_to_official_ratio(arr: np.ndarray, target_ratio: float = 640 / 480) -> np.ndarray:
...
def resize_frame_to_official_lam(frame_rgb: np.ndarray, target_hw=OFFICIAL_LAM_HW) -> np.ndarray:
    ...
    if cropped.shape[:2] != OFFICIAL_WM_HW:
        cropped = cv2.resize(cropped, (OFFICIAL_WM_HW[1], OFFICIAL_WM_HW[0]),
...
def read_shard_row(shard_path: str | Path, row_index: int, columns: Iterable[str] = _NEEDED_COLS) -> dict:
...
# ============================================================================
def derive_fps(timestamp: list[int] | np.ndarray, num_frames: int | None = None) -> float:
...
# ============================================================================
def decode_frames_from_mp4_bytes(mp4_bytes: bytes, frame_idxs: Iterable[int]) -> dict[int, np.ndarray]:
...
def decode_clip_frames(shard_path: str | Path, row_index: int, frame_idxs: Iterable[int],
                       target_hw=OFFICIAL_LAM_HW, resize: bool = True) -> dict[int, np.ndarray]:
...
def decode_window_from_shard(shard_path: str | Path, row_index: int,
                             start_frame: int, stop_frame: int,
                             wm_hw=OFFICIAL_WM_HW) -> np.ndarray:
    ...
    if not frames:
        raise RuntimeError(f"no frames decoded from {shard_path}::{row_index} [{s}, {e})")
...
# ============================================================================
# Contract-compatible replacement for _lam_v6_data.decode_rows_parallel.
...
if __name__ == "__main__":
    ...
    import sys
    sp = sys.argv[1] if len(sys.argv) > 1 else str(resolve_shard_path("agibot_beta", "shard-00000.parquet"))
    row = read_shard_row(sp, 0)
    fps = derive_fps(row["timestamp"], row["num_frames"])
    skills, actions = clip_verbs_raw(row)
    print(f"num_frames={row['num_frames']} fps={fps:.2f} skills={skills[:3]} actions={actions[:1]}")
