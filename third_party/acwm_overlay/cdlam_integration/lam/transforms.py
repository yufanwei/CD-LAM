"""Synthetic camera transforms for LAM baseline same-z finetune.
...
def apply_transform(img: torch.Tensor, meta: Dict) -> torch.Tensor:
    ...
    if persp_idx.numel() > 0:
        ...
        for k_i in persp_idx.tolist():
            ...
            try:
                from torchvision.transforms.core.functional import perspective as tv_persp
    ...
    if blur_idx.numel() > 0:
        try:
            from torchvision.transforms.core.functional import gaussian_blur
            out_b = out.clone()
