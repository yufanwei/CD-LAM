"""LAM training — Stage-1 losses (plan §6.6 / §6.4 / §6.7).
...
def centered_supcon_loss(
    ...
    Args:
      ...
      hard_neg_mask: optional ``(B, B)`` bool. When provided alongside
                     ...
                     these specific negatives. The standard use (earlier recipe, plan §6.6)
      ...
      hard_neg_weight: scalar ≥ 1.0. Default 1.0 = unweighted (earlier recipe behaviour).
...
def build_supcon_masks_from_meta(
    ...
    - **hard_neg_mask** (NEW in earlier recipe): the SUBSET of ``valid_neg`` that is also
      same-episode — these are the "diff-primitive same-episode" pairs that earlier recipe
    ...
    # episode but differs in primitive. earlier recipe final eval showed
...
# =============== Hard-neg triplet auxiliary (earlier recipe, REPORT §6.5 Option B) ====
...
def triplet_hardneg_aux_loss(
    ...
    This is exactly the baseline triplet form (which got gap=+0.065) but operating
    on EMA-centered, normalized z. earlier recipe/earlier recipe/earlier recipe/earlier recipe used SupCon InfoNCE which
...
def masked_mse_full_inter(
    ...
    # We assume T-1 == 1 (LAM core/training predicts a single future frame). Generalize
