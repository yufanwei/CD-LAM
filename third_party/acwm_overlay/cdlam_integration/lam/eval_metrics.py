...
Runs the encoders specified by --encoders (e.g. CD_LAM_BASE, CD_LAM,
...
Usage:
    python cdlam_integration/tools/run_action_metric_probe.py \\
        --pair-index outputs/cdlam_data/pair_index_scored.parquet \\
        --out outputs/cdlam_probe \\
        --encoders CD_LAM_BASE CD_LAM DINOv2_diff RAFT_pooled \\
        --n-per-primitive 200
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--encoders",
        nargs="+",
        default=["CD_LAM_BASE", "CD_LAM", "DINOv2_diff", "RAFT_pooled"],
    )
    ...
    encoders = load_encoders(tuple(args.encoders), device=device)
    ...
    md = [
        "# CD-LAM Phase 5 probe — pre-training",
        "",
