...
When sigmoid(alpha)=0 (init), z_new == z_mu, so baseline LAM's identity / fake-camera
...
Usage (4 GPUs in parallel for variants):
    CUDA_VISIBLE_DEVICES=N python cdlam_integration/tools/train_lam_residual_adapter.py \\
        ...
        --base-lam CD_LAM \\
        --init-from outputs/cdlam_train/T0_R2_anti_collapse/checkpoints/final.pt \\
        --out outputs/cdlam_train/T1_residual_<variant> \\
        --readout-mode R2 \\
