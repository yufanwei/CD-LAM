...
REPO = Path(os.environ.get("CDLAM_ACWM_ROOT", str(Path(__file__).resolve().parents[3])))
H, W = 240, 320
ACTION_DIM = 22
LATENT_DIM = 32
...
def cpu_selftest():
    ...
    np.random.seed(0)
    N, nlive = 1200, 16
    live = list(range(nlive))

    A = np.random.randn(N, ACTION_DIM).astype(np.float32)
    A[:, 16:] = 0.0
    W_true = np.random.randn(ACTION_DIM, LATENT_DIM).astype(np.float32)
    Z = (A @ W_true)[:, :LATENT_DIM]
    Z[:, 16:] += np.random.randn(N, LATENT_DIM - 16) * 3.0
    Z = Z.astype(np.float32)
    am, asd = A[:600].mean(0), A[:600].std(0)
    asd[asd < 1e-6] = 1.0
    zm, zsd = Z[:600].mean(0), Z[:600].std(0)
    zsd[zsd < 1e-6] = 1.0
    An = ((A - am) / asd).astype(np.float32)
    Zn = ((Z - zm) / zsd).astype(np.float32)
    tr, te = np.arange(0, 900), np.arange(900, N)
    ...
    assert len(trend) >= 3, "trend must contain evaluation points"
    last = trend[-1]

    assert last["cyc_R2"] > trend[0]["cyc_R2"], f"cycle did not improve: {trend}"

    with torch.no_grad():
        at = torch.as_tensor(An)
        zt = torch.as_tensor(Zn)
        zhat = g(at).numpy()
        dec = h(zt).numpy()
    assert zhat.shape == (N, LATENT_DIM) and dec.shape == (N, nlive)
    m_az, _ = mmd2_rbf_np(zhat[te], Zn[te])
    m_za, _ = mmd2_rbf_np(dec[te], An[te][:, live])
    assert np.isfinite(m_az) and np.isfinite(m_za), "MMD is not finite"

    x = torch.randn(64, LATENT_DIM, requires_grad=True)
    y = torch.randn(64, LATENT_DIM)
    loss = mmd2_rbf_torch(x, y)
    loss.backward()
    assert x.grad is not None and torch.isfinite(loss), "torch MMD backward failed"

    print(
        f"\n[a22z] CPU SELF-TEST PASSED  "
        f"toy: a->z R2={last['a2z_R2']:.3f} cycle R2={last['cyc_R2']:.3f} "
        f"z->a R2={last['z2a_R2']:.3f} | MMD a->z={m_az:.4f} z->a={m_za:.4f}"
    )
...
def main():
    ...
    ap.add_argument(
        "--cpu-selftest",
        action="store_true",
        help="Run the logic-only smoke test without GPU or dataset access.",
    )
    ap.add_argument(
        "--lam-ckpt",
        default=str(
            REPO
            / "outputs/lam_v7_train/B_with_mask_20260515_102955/checkpoints/step_000300.pt"
        ),
    )
    ap.add_argument(
        "--parquet",
        default=str(REPO / "outputs/lam_v7_data/pair_index_agibot_action_v2.parquet"),
    )
    ap.add_argument(
        "--n-pairs",
        type=int,
    ...
    ap.add_argument("--out", default=str(REPO / "New LAM/results/a22z_frozen"))
    ap.add_argument(
        "--smoke",
        action="store_true",
    ...
    sys.path.insert(0, str(REPO / "training_scope/LAM/V7/tools"))
    import _lam_alias_shim as shim  # noqa: E402

    shim.install()
    import pandas as pd  # noqa: E402
    from LAM_V2.tools.train_lam_action_readout import build_lam  # noqa: E402
    from LAM_V2.tools._lam_v2_forward import encode_full, decode_full  # noqa: E402
    from LAM_V5.tools.train_lam_v5 import load_init_ckpt  # noqa: E402
    from LAM_V6.tools._lam_v6_data import decode_rows_parallel  # noqa: E402
    from probe_idm_upper_bound import encode_trunks  # noqa: E402
