"""LAM v3 — V4 A1 Action Representation Loss (SigLIP-style pairwise sigmoid).

Implements the V4 redesign documented in ``LAM_V3/V4.md`` §1-§17 and the
matching agent_note ``report/LAM_v3/agent_notes/2026-05-08_v4_a1_spec.md``.
...
* :func:`siglip_action_loss` — pairwise sigmoid contrastive loss with explicit
  ...
  *separately* (V4 §17) so that scarce positives are not drowned by abundant
  negatives.

* :func:`build_siglip_graph_from_meta` — constructs the ``(B, B)``
  ``valid_pos``, ``valid_neg``, ``neg_weight`` masks from per-row metadata
  already present in ``pair_index_train.parquet``. Implements the V4 §6/§7
...
"siglip"``; ``"supcon"`` keeps the v0.1 path alive for A/B comparison.
"""
...
def build_siglip_graph_from_meta(
    ...
    * **Ignore (implicit)**: anything not in pos or neg. In particular:
      ...
      - ``same prim AND same episode`` → ignore (could be different phases of
        the same action; V4 §7 single-segment safety).
    ...
    * **Negative weight**: opposite-primitive pairs (open↔close etc.) get
      ...
      v0.1 lever) get ``w_hard_neg`` boost. Both can apply (open vs close in
    ...
    # same-episode hard-neg boost (the v0.1 lever, now per-pair multiplicative
...
def siglip_action_loss(
    ...
    it is ``softplus(s_ij)``. Pairs in neither mask are ignored (V4 §7 ignore).

    Pos and neg terms are normalized *separately* (V4 §17 "positive >
    ...
    if n_pos_real == 0:
        ...
        # negatives (V4 design says ignore, not push apart, when we don't know).
        ...
        # v4_a1_1500. (`(s * 0).sum()` is mathematically zero but has graph
