"""LAM v3 — Route A data layer.

Two pieces:

* :class:`A1Sampler` — primitive-balanced sampler over Route A1 ``pair_index_train.parquet``
  (columns produced by ``LAM_V2/tools/build_a1_pair_index.py``). Compared to F1Sampler
  the differences are:
...
class A1Sampler:
    ...
    * ``mask``      — ``B_mask`` rows from *robosam_mask_training_eligible*; used
    ...
    def sample(
        ...
        v0.3 change (2026-05-08): ``B_hardneg_pairs`` controls explicit hard-neg
        ...
        non-zero entries. v0.1/v0.2 had this set to 0, so the hard-neg weighting
