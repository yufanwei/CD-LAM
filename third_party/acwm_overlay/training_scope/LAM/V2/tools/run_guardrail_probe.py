...
for the LAM v2 readout head. Mirrors the v1 LAM camera-clean criteria from
...
Usage:
    python LAM_V2/tools/run_guardrail_probe.py \\
        --pair-index outputs/lam_v2_data/pair_index_scored.parquet \\
        --head-ckpt outputs/lam_v2_train/T0_R2_anti_collapse/checkpoints/final.pt \\
        --base-lam CD_LAM_V1 \\
        --out outputs/lam_v2_probe/guardrail_T0_R2_anti_collapse.json \\
        --n-pairs 800 --device cuda
"""
...
from LAM_V2.tools.run_action_metric_probe import sample_balanced, decode_pairs_parallel  # noqa: E402
...
from LAM_V2.tools.train_lam_residual_adapter import ResidualAdapter  # noqa: E402


# Camera magnitudes — same defaults as exp_lam_benchmark/run_g4_g2_oldvsv1.
CAMERA_TESTS = [
    ("shift_x", 0.10),
    ("shift_y", 0.05),
    ("zoom", 0.10),
...
def main() -> int:
    ...
    ap.add_argument("--base-lam", default="CD_LAM_V1")
    ...
    lam = build_lam(args.base_lam, device=args.device)
    head = None
    if args.head_ckpt:
        ...
        if is_residual:
            ...
            head = ResidualAdapter(mode=mode, alpha_init=alpha_init).to(args.device)
    ...
    z_mu_real, z_metric_real = encode_pairs(lam, head, pairs, device=args.device)
    ...
    z_mu_id, z_metric_id = encode_pairs(lam, head, id_pairs, device=args.device)
    out["tests"]["identity"] = {
    ...
    for kind, mag in CAMERA_TESTS:
        cam_a = apply_camera_torch(frames_a, kind, mag, device=args.device)
        fake_pairs = np.stack([frames_a, cam_a], axis=1)
        z_mu_fk, z_metric_fk = encode_pairs(lam, head, fake_pairs, device=args.device)
    ...
    for kind, mag in CAMERA_TESTS[:2]:  # limit to shifts to keep budget manageable
        cam_b = apply_camera_torch(frames_b, kind, mag, device=args.device)
        ss_pairs = np.stack([frames_a, cam_b], axis=1)
        z_mu_ss, z_metric_ss = encode_pairs(lam, head, ss_pairs, device=args.device)
