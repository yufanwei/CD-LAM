#!/usr/bin/env python3
"""G1_joint_full_newlatent_v0 trainer
====================================

Mainline as of 2026-05-06 (replacing F1 frozen-decoder line):

  - encoder + decoder + patch_up + action_up ALL trainable (joint)
  - From v1 camera-clean ckpt as init
  ...
  - 4-card DDP via torchrun --nproc_per_node=4, manual all_reduce of grads
    (consistent with F1 trainer; our forward goes through custom encode/decode).
...
For G1_no_gap control, just set --w-gap-target 0.0 (no schedule needed).

Usage (preflight):
    torchrun --nproc_per_node=4 --master_port=29501 \\
      LAM_V2/tools/train_lam_v2_g1.py \\
        --config LAM_V2/configs/g1_joint_full_newlatent_v0.yaml \\
        --out outputs/lam_v2_train/G1_preflight \\
        --m0-baseline outputs/lam_v2_train/F1_all_encoder_gen_gap_guard/M0_baseline.json \\
...
Usage (main):
    torchrun --nproc_per_node=4 --master_port=29501 \\
      LAM_V2/tools/train_lam_v2_g1.py \\
        --config LAM_V2/configs/g1_joint_full_newlatent_v0.yaml \\
        --out outputs/lam_v2_train/G1_joint_full_newlatent_v0 \\
        --m0-baseline outputs/lam_v2_train/F1_all_encoder_gen_gap_guard/M0_baseline.json
"""
...
# Reuse F1's sampler + decode + DDP helpers.
...
from LAM_V2.tools.train_lam_v2_f1 import (  # noqa: E402
    F1Sampler as V2Sampler,  # same sampler logic, real + identity + triplets
...
# =============== Param groups (G1-specific) ================================


def collect_param_groups_g1(lam_inner: torch.nn.Module, lr_cfg: dict) -> List[dict]:
    """Five-group LR partition (task book §3):
    ...
    LR ordering (task book §3): head ≥ late_encoder > mid/early > decoder
...
def make_state(args, world: int, rank: int, local_rank: int) -> dict:
    return {
        ...
        "trainer": "G1_joint_full_newlatent_v1",
    }
...
def train(args, cfg):
    ...
    if is_main:
        ...
        shutil.copy2(args.config, out_dir / "configs" / "g1_config.yaml.copy")
    if world > 1:
        dist.barrier()
    ...
    # in G1 v0 at step 500 (KL 77 → 0.98, z_norm 6.4 → 1.1, usage_gap → 0).
    ...
    # ----- M0 baseline (for stop-condition floors)
    ...
    # ----- build trainable LAM (init from v1)
    log("building trainable G1 LAM (init from v1 ckpt) ...")
    lam_train = build_lam("CD_LAM_V1", device=device)
    lam_inner = lam_train.lam
    lam_inner.train()

    # decoder is TRAINABLE in G1; nothing frozen. (No lam_ref needed — task book §13
    # says don't strong-distill to v1; we use v1 only as init.)
    ...
    try:
        while step < total_steps:
            ...
            if step == warmup_steps and warmup_steps > 0:
                ...
                # compare against baseline (M0 v1 z_norm)
                if m0_z_norm and cur_z_norm < 0.4 * m0_z_norm:
                    gate_issues.append(
                        f"z_norm {cur_z_norm:.2f} < 0.4 × M0 ({0.4 * m0_z_norm:.2f}) — z collapsed in warmup"
                    )
            ...
            if (step > 0 and step % eval_every == 0) or step == total_steps - 1:
                if is_main:
                    ...
                    if ckpt_path is None:
                        ...
                        f1_baseline = cad.get("f1_v3_baseline_ckpt", None)
                        if f1_baseline:
                            cmd += ["--f1-baseline-ckpt", str(f1_baseline)]
