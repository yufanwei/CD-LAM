#!/usr/bin/env python3
"""Unified LAM v2 eval — runs the M0 baseline metric suite on any encoder.

Encoder selection:
    --encoder-mode v1            : use v1 LAM (build_lam("CD_LAM_V1"))
...
Usage:
    # M0 baseline on v1
    python LAM_V2/tools/eval_lam_v2_full.py \
        --pair-index outputs/lam_v2_data_v2/pair_index_val.parquet \
        --out outputs/lam_v2_train/F1_all_encoder_gen_gap_guard/M0_baseline.json \
        --encoder-mode v1 \
...
from LAM_V2.tools._lam_v2_forward import encode_full  # noqa: E402
...
def build_encoder(args, device: str):
    if args.encoder_mode == "v1":
        lam = build_lam("CD_LAM_V1", device=device)
        return lam.lam, "v1"
    elif args.encoder_mode == "f1":
        # build skeleton then load F1 ckpt over it
        lam = build_lam("CD_LAM_V1", device=device)  # gives correct architecture
        ck = torch.load(args.ckpt, map_location="cpu", weights_only=False)
        sd = ck.get("model", ck.get("state_dict", ck))
        # Strip "lam." prefix if present (matching how v1 was saved)
...
def main() -> int:
    ...
    ap.add_argument("--encoder-mode", choices=["v1", "f1"], default="v1")
    ...
    lam_inner, enc_tag = build_encoder(args, device=args.device)

    # ---- encode real
    print(f"[eval] encoding {len(real_pairs)} real pairs ...", flush=True)
    t0 = time.time()
    z_real = encode_lam(lam_inner, real_pairs, device=args.device, batch=64)
    ...
    z_id = encode_lam(lam_inner, id_pairs, device=args.device, batch=64)
    ...
    for kind, mag in CAMERA_TESTS_FAKE:
        ...
        z_fk = encode_lam(lam_inner, fake_pairs, device=args.device, batch=64)
    ...
    for kind, mag in CAMERA_TESTS_SS:
        ...
        z_ss = encode_lam(lam_inner, ss_pairs, device=args.device, batch=64)
