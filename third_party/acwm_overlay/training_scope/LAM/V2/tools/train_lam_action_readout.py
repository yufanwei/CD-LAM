...
Sampler (Phase 6):
    ...
    LAM_V2/configs/samplers/action_metric_balanced.yaml).

Outputs:
    outputs/lam_v2_train/T0_readout/{config.yaml, train_log.jsonl,
                                     checkpoints/, eval_summary.md}

Usage:
    python LAM_V2/tools/train_lam_action_readout.py \\
        ...
        --base-lam CD_LAM_V1 \\
        --out outputs/lam_v2_train/T0_readout \\
...
LAM_NAME_ALIASES = {
    ...
    "DreamDojo_LAM_v1": "CD_LAM_V1",
}
LAM_NAME_CHOICES = tuple(sorted({"CD_LAM_BASE", "CD_LAM_V1", *LAM_NAME_ALIASES}))
...
def build_lam(name: str, device: str = "cuda") -> nn.Module:
    ...
    if name == "CD_LAM_BASE":
        ...
        ckpt = REPO / "outputs/lam_v1_samez_camera_smoke_8h/checkpoints/step_009800.pt"
        m = LAM(
            image_channels=3,
            lam_model_dim=1024,
            lam_latent_dim=32,
        ...
        print(
            f"[lam] v1 loaded with {len(missing)} missing / {len(unexpected)} unexpected",
            flush=True,
        )
    else:
...
def keep_anchor_loss(
    ...
    """Hinge: penalize ||z_new - z_v1|| / ||z_v1|| above tau. Light anchor to v1 final_z."""
