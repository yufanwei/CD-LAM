#!/usr/bin/env python3
"""F1 — all-encoder LAM v2 main training (task book §1, §6, §9, §10, §11).
...
Frozen:
    ...
    v1 reference LAM (separate copy held on same device)

Loss (per real-batch step):
    L_gen   = MSE(recon, gt) + beta * KL[sum-dim, mean-batch]
    L_id    = mean(||z_id||^2) / s,  s = EMA(mean(||z_v1_real||^2))
    L_gap   = mean(max(0, margin - cos(z_a, z_p) + cos(z_a, z_n)))
    L_norm  = mean((||z_mu|| / (||z_v1|| + eps) - 1)^2)
    L_trust = mean(max(0, d - tau)^2),  d = ||z_mu - sg(z_v1)|| / (||z_v1|| + eps)
...
Usage:
    ...
    python LAM_V2/tools/train_lam_v2_f1.py \
        --config LAM_V2/configs/f1_all_encoder_gen_gap_guard.yaml \
        --out outputs/lam_v2_train/F1_dryrun \
    ...
    python LAM_V2/tools/train_lam_v2_f1.py \
        --config LAM_V2/configs/f1_all_encoder_gen_gap_guard.yaml \
        --out outputs/lam_v2_train/F1_all_encoder_gen_gap_guard \
        --total-steps 10000
"""
...
from LAM_V2.tools._lam_v2_forward import forward_full, encode_full  # noqa: E402
...
def collect_param_groups(
    ...
    """Layer-wise LR groups (task book §11).
...
def train(args, cfg):
    ...
    log("building trainable F1 LAM (init from v1 ckpt) ...")
    lam_train = build_lam("CD_LAM_V1", device=device)
    lam_inner = lam_train.lam
    lam_inner.train()

    log("building frozen v1 reference LAM ...")
    lam_ref = build_lam("CD_LAM_V1", device=device)
    lam_ref.eval()
    ...
    # ---- EMA scale s for L_id (norm of v1 z on real batch)
    s_ema = None
    s_ema_alpha = 0.95
    ...
    try:
        while step < total_steps:
            ...
            # Tighter clip than v3's 5.0 — needed at larger effective batch where
            ...
            if (step > 0 and step % eval_every == 0) or step == total_steps - 1:
                if is_main:
                    ...
                    if ckpt_path is None:
                        ...
                        cmd = [
                            sys.executable,
                            str(REPO / "LAM_V2/tools/eval_lam_v2_full.py"),
                            "--pair-index",
