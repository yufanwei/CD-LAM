"""LAM v4.2+ loss / graph extensions.

Adds three new pieces on top of the V4 A1 / v4.1 module
(``LAM_V3.tools._lam_v3_v4_losses``):

* :func:`build_siglip_graph_v42` — same rules as V4.1 plus:
...
Per-V4 design (V4.md §17), positive cloud consolidation is the next lever after
v4.1 already pushed negatives apart. These hooks are designed to be additive on
top of v4.1 — leaving everything default reproduces v4.1 behaviour.
"""
...
from LAM_V3.tools._lam_v3_v4_losses import (  # noqa: F401
    SigLIPHead,
    OPPOSITE_PAIRS,
...
def build_siglip_graph_v42(
    ...
    *,
    ...
    w_pos_cross_dataset: float = 1.0,            # NEW (v4.2): boost on same-prim cross-ds positives
    neg_weight_mode: str = "multiplicative",     # NEW (v4.2): {"multiplicative", "max"}
    w_neg_cap: float = float("inf"),             # NEW (v4.2): final clamp on neg_weight
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, dict]:
    """Same rules as V4.1 ``build_siglip_graph_from_meta`` plus three v4.2 hooks.

    Returns ``(valid_pos, valid_neg, pos_weight, neg_weight, stats)``.
    Default kwargs reproduce v4.1 behavior with ``pos_weight = valid_pos.float()``
    (uniform 1.0) and ``neg_weight = multiplicative opposite·hard_neg`` (matches
    v4.1 / V4 A1 multiplicative mode).
    ...
    * ``w_pos_cross_dataset > 1.0``: positive pairs whose two endpoints belong
      ...
      which v4.1 dashboard showed are the leakage@5 ≈ 0.91 weak spot.
    * ``neg_weight_mode = "max"``: instead of ``opp × hard_neg`` (multiplicative,
      which gave weight 3.0 to opposite-AND-same-ep pairs in v4.1), use
      ``max(opp_weight, hard_neg_weight)`` (capped at ``w_opposite``).
      v4.1 metrics suggested neg pressure was already saturating; capping makes
...
def siglip_action_loss_split(
    ...
    *,
    ...
    Replaces ``siglip_action_loss`` for v4.3+. The loss is
    ...
    same per-side normalization as v4.1 ``siglip_action_loss``, just with the
...
def build_neg_subgroup_masks(
    ...
    """V4.6 — split valid_neg into 3 disjoint subgroups for separate λ scheduling.
...
def siglip_action_loss_v46(
    ...
    *,
    ...
    """V4.6 — structured siglip loss with 5 separate λ knobs.
...
def xds_ranking_centroid_loss(
    ...
    *,
    ...
    """V4.6 — ranking-style xds centroid loss (replaces v4.5 raw alignment).
...
def primitive_separation_loss(
    ...
    *,
    ...
    """V4.6 — prototype (primitive centroid) separation loss.
...
def conditional_xds_centroid_loss(
    ...
    *,
    ...
    """V4.5 — conditional cross-dataset centroid alignment.
...
class MidLayerSigLIPHead(nn.Module):
    """V4.6 — projection head for mid-layer (block 6) features.
...
__all__ = [
    ...
    "build_siglip_graph_v42", "siglip_action_loss_split",
    "hard_positive_reweight",
    "conditional_xds_centroid_loss", "MidLayerSigLIPHead",
    # v4.6 additions
    "build_neg_subgroup_masks", "siglip_action_loss_v46",
