"""LAM v4.2+ data layer extensions.

:class:`A2Sampler` extends :class:`LAM_V3.tools._lam_v3_data.A1Sampler` with:
...
Default flags reproduce A1Sampler behavior (back-compat).

Per LAM_v4 dashboard finding: leakage@5 ≈ 0.91 across all v3/v4 ckpts is the
...
from LAM_V3.tools._lam_v3_data import A1Sampler, _ep_key


class A2Sampler(A1Sampler):
    """Dataset-balanced extension of A1Sampler (v4.2+).

    Adds two flags to :meth:`__init__`:

    * ``dataset_balanced``: when True, ``_sample_real`` enforces dataset balance
      across the chosen primitives. Default False ⇒ same as A1Sampler.
    * ``min_cross_dataset_per_primitive``: when ≥ 1 and ``dataset_balanced`` is
    ...
    def _sample_real(self, B_real: int, B_hardneg_pairs: int = 0) -> List[int]:
        """Same signature as A1Sampler._sample_real but dataset-balanced when
