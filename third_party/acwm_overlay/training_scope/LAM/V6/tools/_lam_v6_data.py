"""Data helpers for LAM V6 no-label view-contrastive training.

The V6 sampler mirrors the official LAM training data pipeline at
...
data path) so V6 latent action is aligned with what the downstream Cosmos WM
...
V6 loss is computed on top of the produced pair. Per-row mask validity (for
...
from LAM_V5.tools._lam_v5_data import FgBgMaskCache  # noqa: E402
...
def resize_frame_to_official_lam(
    ...
    instead of one ~4.5x step. We replicate that here so V6 training, V6 eval,
...
def decode_rows_parallel(
    ...
    z from a 240x320 ``lam_video`` derived after the same 4:3 center crop. V6
...
def decode_rows_random_stride(
    ...
    """Parallel decode for ``V6VideoSampler`` output rows.
...
class V6VideoSampler:
    ...
    Unlike ``V6PairSampler`` this sampler does not depend on a pre-built
...
class V6PairSampler:
    """Random transition sampler for V6.
    ...
    def __init__(
        ...
        *,
        ...
        # Do NOT filter on valid_relation_pair. In the V2/A1 pair index that
        # column means "usable for the labelled relation graph", so it depends
        # on label-derived pools. V6's no-label claim would be invalid if the
        ...
        df = df.reset_index(drop=True)
        if len(df) == 0:
            raise RuntimeError(f"no usable V6 rows in {pair_index_path}")
...
def load_fg_bg_masks_for_v6_rows(
    rows: pd.DataFrame,
    cache: FgBgMaskCache,
    ...
    *,
    ...
    """Load foreground/background masks for V6 rows at ``frame_col`` (default ``frame_j``).

    Works for both ``V6PairSampler`` rows (columns ``robosam_interaction_mask_path``,
    ``frame_j``) and ``V6VideoSampler`` rows (column ``mask_npz_path``, ``frame_j``
    ...
    **runtime_dilate_px**: optional extra dilation kernel applied to the
    ...
    for i, r in enumerate(rows.itertuples(index=False)):
        path = ""
        # V6VideoSampler unified column name.
