"""LAM v5 trainer: fg/bg reconstruction + SigLIP action geometry.

V5 keeps the V4.1 SigLIP action head but replaces the older reconstruction
stack with RoboSAM fg/bg supervision:

  L_total = L_rec_fg + gamma_bg · L_rec_bg + beta_kl · KL_freebit
          + lambda_sig · L_sig + w_id · L_id

The v5.2 production path adds ``partial_fullmix``:
...
Reuses v2/v3/v4 infrastructure:
  - ``LAM_V2/tools/_lam_v2_forward.py`` (encode_full / decode_full / forward_full)
  - ``LAM_V2/tools/train_lam_action_readout.build_lam`` (init from v1 ckpt)
  - ``LAM_V2/tools/train_lam_v2_f1.{init_distributed, all_reduce_grads, write_run_state}``
  - ``LAM_V2/tools/train_lam_v2_g1.{collect_param_groups_g1, kl_loss_free_bits, _param_group_norms}``
  - ``LAM_V3`` sampler and centered-action helpers
  - ``LAM_V4`` SigLIP loss variants and dashboard/eval helpers
...
Usage (preflight):
    torchrun --nproc_per_node=4 --master_port=29511 \
      LAM_V5/tools/train_lam_v5.py \
        --config LAM_V5/configs/v5_2_partial_fullmix.yaml \
        --out outputs/lam_v5_train/v5_2_preflight \
...
Usage (main):
    torchrun --nproc_per_node=4 --master_port=29511 \
      LAM_V5/tools/train_lam_v5.py \
        --config LAM_V5/configs/v5_2_partial_fullmix.yaml \
        --out outputs/lam_v5_train/v5_2_main_1500
"""
...
from LAM_V2.tools.train_lam_v2_f1 import (  # noqa: E402
    init_distributed,
    all_reduce_grads,
    write_run_state,
)
from LAM_V2.tools.train_lam_v2_g1 import (  # noqa: E402
    collect_param_groups_g1,
    kl_loss_free_bits,
    _param_group_norms,
)

from LAM_V3.tools._lam_v3_data import (  # noqa: E402
    A1Sampler,
    MaskCache,
    decode_rows_parallel,
    load_interaction_masks_for_rows,
)
from LAM_V5.tools._lam_v5_data import (  # noqa: E402
    FgBgMaskCache,
    load_fg_bg_masks_for_rows,
)
from LAM_V5.tools._lam_v5_losses import (  # noqa: E402
    fg_weighted_reconstruction,
    bg_consistency_reconstruction,
)
from LAM_V3.tools._lam_v3_losses import (  # noqa: E402
    EMACenterer,
...
from LAM_V3.tools._lam_v3_v4_losses import (  # noqa: E402
    SigLIPHead,
    siglip_action_loss,
    build_siglip_graph_from_meta,
    opposite_pair_id_table,
)
from LAM_V4.tools._lam_v4_losses import (  # noqa: E402
    build_siglip_graph_v42,
    siglip_action_loss_split,
    hard_positive_reweight,
    conditional_xds_centroid_loss,
    build_neg_subgroup_masks,
    siglip_action_loss_v46,
    xds_ranking_centroid_loss,
    primitive_separation_loss,
)
from LAM_V4.tools._lam_v4_data import A2Sampler  # noqa: E402
...
def load_init_ckpt(
    ...
    * G1/v5 format: top-level "model" key, weights prefixed with "lam."
...
def train(args, cfg) -> None:
    ...
    # v0.3: hard-neg pair injection. n_pairs=8 → last 16 rows of real_idx are
    # 8 (anchor, hardneg) pairs from multi-prim episodes. v0/v0.1/v0.2 had this
    ...
    # ---- V5 config (2026-05-09) — minimalist 4-loss path:
    # L_total = L_rec_fg + γ·L_rec_bg + β·KL + λ·L_sig
    # When v5_mode is True, the trainer skips L_id, L_use_full, L_use_inter,
    # L_aux_triplet, L_rec_inter (v4 mask block) and uses fg/bg-weighted
    # reconstruction instead. Sampler is restricted to mask_eligible rows so
    # every batch row has a valid fg+bg mask.
    v5_mode = bool(loss_cfg.get("v5_mode", False))
    v5_lambda_fg = float(loss_cfg.get("v5_lambda_fg", 1.0))  # core L_rec_fg coefficient
    v5_gamma_bg = float(loss_cfg.get("v5_gamma_bg", 0.1))  # γ — bg consistency weight
    v5_min_pixels_fg = int(loss_cfg.get("v5_min_pixels_fg", 64))
    v5_min_pixels_bg = int(loss_cfg.get("v5_min_pixels_bg", 64))
    # ---- V5.2 (2026-05-09 16:30) — partial_fullmix:
    # Diagnostic 16:10 confirmed v5.0/5.1 train/eval distribution mismatch is the
    ...
    v5_partial_fullmix = bool(loss_cfg.get("v5_partial_fullmix", False))
    v5_rho_full = float(loss_cfg.get("v5_rho_full", 0.3))  # ρ on unmasked rows
    ...
    # ---- hard_neg_weight schedule (v0.2): optionally decay weight from
    #      `init` to `target` across `[ramp_end_step, decay_end_step]`. v0.1
    ...
    # v0.4 (REPORT §6.5 Option B): triplet-style auxiliary on hard-neg pairs.
    # SupCon optimizes class log-prob, NOT the cos(a,p)-cos(a,n) margin. F1_v3
    # used triplet hinge directly and got gap=+0.065. v0.4 keeps SupCon as main
    # action signal (top1 win) but adds small triplet aux on (anchor, pos, hard-neg)
    # to put gap back as a first-order target.
    # 0 = disabled (v0/v0.1/v0.2/v0.3 behaviour). 0.05-0.20 reasonable.
    ...
    # ---- V4 A1: action_loss_kind switch (siglip / supcon / none) ----
    ...
    # ---- V4.1 (2026-05-08): lambda_sig cosine decay in main phase ----
    # V4 A1 ablation showed eff_rank kept dropping (27.07 → 25.24) under a
    ...
    # (cosine schedule). When end <= ramp_end, behaves as flat (V4 A1 baseline).
    ...
    # ---- V4.1: head capacity + per-head LR (was hard-coded to G_head LR) ----
    ...
    # ---- V4.2 (2026-05-08) extensions: cross-dataset positive boost +
    # neg combination mode (multiplicative vs max) + neg cap + sampler dataset
    # balance + L_pos / L_neg split schedule. Defaults reproduce v4.1.
    siglip_use_v42_graph = bool(loss_cfg.get("siglip_use_v42_graph", False))
    ...
    # ---- V4.4: hard-positive mining (high-conf cos<τ_pos boost) ----
    ...
    # ---- V4.5: conditional cross-dataset centroid loss ----
    ...
    # ---- V4.6: structured negatives + ranking xds + prototype separation ----
    # When `siglip_action_loss_v46_enabled` = True, the SigLIP loss uses
    # siglip_action_loss_v46 (5 separate λ for opp/hard/rand neg + same/xds pos)
    ...
    siglip_action_loss_v46_enabled = bool(
        loss_cfg.get("siglip_action_loss_v46_enabled", False)
    )
    ...
    # ranking xds (replaces v4.5 raw centroid pull)
    ...
    # ---- M0 baseline (from G1's v1 baseline json — same architecture, comparable)
    ...
    if m0:
        log(
            f"M0 baseline z_norm={m0['z_geometry']['z_mu_norm_mean']:.3f} "
    ...
    if sampler_dataset_balanced:
        log(
            f"sampler: A2Sampler (primitive×dataset balanced, min_xds_per_prim={sampler_min_xds_per_prim})"
        )
        sampler = A2Sampler(
    ...
    # ---- mask cache (v4 path) + fg+bg cache (v5 path)
    ...
    # ---- v5: when v5_mode AND NOT v5_partial_fullmix, restrict sampler.real to mask_eligible.
    # v5.2 partial_fullmix path keeps full pair_index — fg/bg loss only on rows where mask
    # is valid; unmasked rows get rho·L_rec_full instead.
    if v5_mode and not v5_partial_fullmix:
        eligible = (
            sampler.real["robosam_mask_training_eligible"].fillna(False).astype(bool)
        )
        if eligible.sum() < 1000:
            raise RuntimeError(
                f"v5_mode requires sampler.real with mask_eligible rows; got "
                f"only {int(eligible.sum())} eligible rows."
            )
        # rebuild internal indices in-place on the filtered DataFrame
        from collections import defaultdict
        from LAM_V3.tools._lam_v3_data import _ep_key
        ...
        if hasattr(sampler, "by_prim_ds"):
            # rebuild A2Sampler additional indices
        ...
        log(
            f"v5_mode: sampler.real restricted to mask_eligible — "
            f"{len(sampler.real):,} rows, {len(sampler.primitives)} primitives, "
            f"{len(sampler.hard_neg_episodes)} hard-neg episodes"
        )

    # ---- build trainable LAM
    log("building trainable v3 LAM (init from v1 → optionally overlay --init-ckpt) ...")
    lam_train = build_lam("CD_LAM_V1", device=device)
    ...
    # ---- V4 A1: SigLIP head (only when action_loss_kind == "siglip") ----
    ...
    param_groups = collect_param_groups_g1(lam_inner, cfg["trainer"]["lr"])
    if siglip_head is not None:
        # V4.1: dedicated head LR (was hard-coded to G_head LR in V4 A1).
    ...
    def _build_one_batch():
        # In v5_mode: B_mask=0 always (fg/bg loaded per real row instead).
        # B_id is honored even in v5_mode (v5.1 adds L_id back; v5.0 had id_p50=0.564
        # collapse w/o L_id). Set B_id=0 in yaml to disable.
        eff_B_id = B_id
        eff_B_mask = 0 if v5_mode else B_mask
        ...
        # v5: load fg + bg masks for the REAL block (sampler.real is already
        # filtered to mask_eligible in v5_mode init).
        if v5_mode:
    ...
    try:
        while step < total_steps:
            ...
            # v5 fg/bg masks for the REAL block
            if v5_mode:
            ...
            if v5_mode and fg_t is not None:
                ...
                L_rec_bg_t, bg_stats = bg_consistency_reconstruction(
                    ...
                    bg_mask=bg_t,
                    valid=fgbg_v_t,
                    min_pixels=v5_min_pixels_bg,
                )
                v5_rec_stats.update(fg_stats)
                v5_rec_stats.update(bg_stats)

            # ---- V5.2 partial_fullmix: rho·L_rec_full on rows where mask is invalid.
            ...
            if v5_partial_fullmix and fgbg_v_t is not None and fg_t is not None:
            ...
            if v5_mode:
                L_gen = (
                    v5_lambda_fg * L_rec_fg_t
                    + v5_gamma_bg * L_rec_bg_t
                    + v5_rho_full * L_rec_full_unmasked_t
                    + beta_kl * kl_loss
                )
            else:
            ...
            # v5.0 had B_id=0 → L_id off → id_p50=0.564 collapse @ step 500.
            # v5.1 honors B_id even in v5_mode (id forward computed if B_id>0).
            if v_id is not None and v_id.shape[0] > 0:
                ...
                # band and is the form recommended in v5.2 (avoid pushing id_p50 below
            ...
            # v5_mode: L_use disabled (fg-weighted recon already enforces z-use)
            do_l_use = (step % l_use_every == 0) and not v5_mode
            ...
            if action_loss_kind == "siglip":
                # ---- V4 A1: SigLIP graph + pairwise sigmoid loss (V4.md §15)
                ehmp = torch.tensor(
                ...
                # V4.2: dataset hash (used for w_pos_cross_dataset boost)
                ...
                # V4.3+: pos/neg split λ schedule. Both warmup→ramp→cosine decay
                ...
                # V4.4: hard-positive mining — reweight pos_weight up for
                ...
                if siglip_action_loss_v46_enabled:
                    # V4.6: structured negatives (opp/hard/rand) + 5 λ schedules
                    ...
                    L_rel, sig_stats = siglip_action_loss_v46(
                        z_mu=z_mu_real,
                        valid_pos=valid_pos_g,
                        head=siglip_head,
                    ...
                    L_rel, sig_stats = siglip_action_loss(
                        ...
                        head=siglip_head,
                        neg_weight=neg_weight_g,
                        pos_weight=pos_weight_g,  # None when v4.1 graph
                    )
                    # V4.1: outer cosine decay lambda_sig from `target` (at ramp_end)
                ...
                # ---- v0 / v0.1 path: EMA-centered SupCon ----
                ...
                # If decay_end <= ramp_end, behaves as fixed at init (v0/v0.1 behaviour).
            ...
            # When SigLIP split / v46 path is on, all λs already include warmup
            ...
            if action_loss_kind == "siglip" and (
                siglip_action_loss_split_enabled or siglip_action_loss_v46_enabled
            ):
                lam_rel = 1.0
            ...
            # ---- v0.4 triplet aux on hard-neg (REPORT §6.5 Option B): direct
            ...
            if lambda_aux_triplet > 0.0 and step >= warmup_steps and not v5_mode:
            ...
            # ---- V4.5: conditional cross-dataset centroid loss (SigLIP only) ----
            ...
            # ---- V4.6: ranking xds + prototype separation ----
            ...
            # Bug observed 2026-05-08 v4_a1_1500 fixed-run hung at step ~551
            ...
            if step % log_every == 0 or step == total_steps - 1:
                ...
                rec = {
                    ...
                    # ---- V5 reconstruction stats ----
                    "v5_mode": bool(v5_mode),
                    ...
                    "v5_n_fg_rows": int(v5_rec_stats.get("n_fg_rows", 0))
                    if v5_rec_stats
                    else 0,
                    "v5_fg_area_mean": float(v5_rec_stats.get("fg_area_mean", 0.0))
                    if v5_rec_stats
                    else 0.0,
                    "v5_bg_area_mean": float(v5_rec_stats.get("bg_area_mean", 0.0))
                    if v5_rec_stats
                    else 0.0,
                    "v5_lambda_fg": float(v5_lambda_fg),
                    "v5_gamma_bg": float(v5_gamma_bg),
            ...
            if step == warmup_steps and warmup_steps > 0:
                ...
                if m0_z_norm and cur_z_norm < 0.4 * m0_z_norm:
                    gate_issues.append(
                        f"z_norm {cur_z_norm:.2f} < 0.4 × M0 ({0.4 * m0_z_norm:.2f}) — z collapsed"
                    )
            ...
            if (
                ...
                if is_main:
                    ...
                    if ckpt_path_str is None:
                        ...
                        cmd = [
                            sys.executable,
                            str(REPO / "LAM_V2/tools/eval_lam_v2_g1.py"),
                            "--pair-index",
                        ...
                        if cad.get("f1_v3_baseline_ckpt"):
                            cmd += [
                                "--f1-baseline-ckpt",
                                str(cad["f1_v3_baseline_ckpt"]),
                            ]
                        ...
                        try:
                            ...
                            if res.returncode != 0:
                                ...
                                issues: List[str] = []
                                geom = ev.get("g1", {}).get("z_geometry", {})
                                ret = ev.get("g1", {}).get("retrieval", {})
                                idr = ev.get("g1", {}).get(
                                    "identity_ratio_vs_median", {}
                                )
                                ug = ev.get("g1", {}).get("usage_gap", {})
                                if (
                                    idr.get("p50")
                                ...
                                # Route A specifically watches raw pairwise cosine (common-cone)
...
def main() -> int:
    ...
    ap.add_argument(
        "--init-ckpt",
        default=None,
        help="path to G1-format ckpt (model keys 'lam.<...>') used as init; "
        "overrides config trainer.init_ckpt",
    )
    ap.add_argument(
        "--m0-baseline",
        default=None,
        help="path to v1 baseline metrics json for warmup gate / stop conditions",
    )
