"""EXP-2: IDM upper bound — decode EE-action from the encoder's BOTH-frame trunk
features (pooled o_t + o_{t+1} tokens, 2048-D), bypassing the 32-D z bottleneck.

If IDM-EE >> z-EE (0.72), the 32-D z is throwing away EE info -> shaping z (EXP-3)
will help a lot. If IDM-EE ~= z-EE, 0.72 is near the data/pixel limit -> z-shaping
won't help and 0.72 is the answer. Reuses the agibot_beta EE cache.

    CUDA_VISIBLE_DEVICES=3 python training_scope/LAM/V7/tools/probe_idm_upper_bound.py \
      ...
      --out "New LAM/results/idm_upper_bound"
"""
...
from probe_ee_vs_joint import mlp_decode, r2_dims  # noqa: E402
