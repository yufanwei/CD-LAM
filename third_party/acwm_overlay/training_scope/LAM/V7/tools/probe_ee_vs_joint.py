"""EXP-1 keystone: decode JOINT-action vs EE-action from the SAME frozen V7-B z.

Tests H1: is the ~0.44 wall partly a wrong-target artifact (joint angles are FK-
entangled & distal joints are visually invisible; EE motion is what z encodes)?
Encodes z ONCE per pair, then runs M1 (linear ridge) + M2 (conditioned MLP on [z,ctx])
for BOTH targets, episode-disjoint. EE position decoding >> 0.44 => H1 confirmed.

    CUDA_VISIBLE_DEVICES=3 python training_scope/LAM/V7/tools/probe_ee_vs_joint.py \
      --cache outputs/lam_v7_data/agibot_beta_ee_cache.npz \
      --lam-ckpt outputs/lam_v7_train/B_with_mask_20260515_102955/checkpoints/step_000300.pt \
      --out "New LAM/results/ee_vs_joint"
"""
...
sys.path.insert(0, "training_scope/LAM/V7/tools")
...
from _lam_v7_ctx import encode_full_with_trunk  # noqa: E402
...
def main():
    ...
    res = {
        ...
        "n_train": len(tr),
        "n_eval": len(te),
        "interp": "JOINT vs EE decoded from identical frozen V7-B z (agibot_beta, episode-disjoint). "
