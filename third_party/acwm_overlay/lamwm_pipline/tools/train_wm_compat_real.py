...
from lamwm_pipline.src.video_preprocess import (  # noqa: E402
...
def patch_forward_to_use_cached_z(model, z_cached, fix_noise: bool = False):
    ...
    z_cached: (B, T_chunk, 32) bf16 — our v1/old LAM z replicated across the chunk.
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        "off by default for old runs; V7 wrapper enables it.",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        "the LAM encoder z space, via lamwm_pipline.tools.gr_zsource. Requires "
    ...
    ap.add_argument(
        ...
        default=os.environ.get(
            ...
            str(REPO / "New LAM/results/v2_dec2/ckpt_last.pt"),
        ),
    ...
    ap.add_argument(
        "--gr-action-cache",
        default="outputs/lam_v7_data/agibot_beta_ee_cache.npz",
    ...
    ap.add_argument(
        ...
        default=os.environ.get(
            ...
            str(REPO / "New LAM/results/gr_retrained/gr_agibot_beta.pt"),
        ),
    ...
    # ---- ACL gate
    from lamwm_pipline.src.registry import assert_allowed_for_wm
    ...
    if args.z_source == "gr-action":
        ...
        # runs with that SAME LAM (registry id lam_joint_v2dec2 -> this ckpt). Warn loudly if not.
        if args.lam_id != "lam_joint_v2dec2":
            print(
                f"[c0_real][WARN] --z-source gr-action with --lam-id {args.lam_id!r}: g_r's z is in "
                f"the joint LAM (lam_joint_v2dec2) z space; pairing with a different LAM is a "
                f"z-space MISMATCH. Use --lam-id lam_joint_v2dec2 for a faithful Stage-2 run.",
                flush=True,
            )
        ...
        if args.gr_aligned:
            ...
            from lamwm_pipline.tools.gr_aligned_action import (
                load_gr_calibrated,
            ...
            from lamwm_pipline.tools.gr_zsource import (
                load_gr_zsource,
    ...
    if gr_aligned_mode:
        ...
        from lamwm_pipline.src.video_preprocess import (
    ...
    from lamwm_pipline.src.lam_loader import build_encoder

    sys.path.insert(0, str(REPO / "finetune_4-30" / "scripts"))
    from model import pair_uint8_to_float
    ...
    if args.cond_dropout > 0.0:
        from lamwm_pipline.tools.z_usage_monitor import DropoutConfig
    ...
    if args.monitor and is_rank0:
        ...
        from lamwm_pipline.tools.z_usage_monitor import ZUsageMonitor, MonitorConfig
    ...
    try:
        for step in range(args.start_step, args.steps):
            ...
            if dcfg is not None:
                from lamwm_pipline.tools.z_usage_monitor import mask_condition_slots
