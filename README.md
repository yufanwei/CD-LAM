...
[Model repository (weights pending)](https://huggingface.co/yufanwei/CD-LAM)
...
[`results/paper_results.json`](results/paper_results.json).
...
> paper protocol configs are runnable. Model weights and datasets are not
> redistributed yet. The public repository therefore provides a complete CPU
> integration test and a fail-closed adapter interface, but it does not claim
> that a fresh clone can reproduce the paper checkpoints without compatible
> external models, data, and GPU environments.
...
```text
src/cd_lam/
  adapters.py          external model integration boundary
  bridge.py            strict 22D-to-32D bridge loading and inference
  config.py            typed pipeline configuration and path resolution
  data/action.py       action-coordinate transforms and stride contracts
  objectives.py        Stage-1 CD-LAM objectives
  plans.py             deterministic, fail-closed execution plans
  training/            separate Stage-1, Stage-2, bridge, and Stage-3 runners
  metrics/fdce.py      FDCE reduction from prepared tracks
configs/               2B/14B paper protocol templates
docs/                  training, data, checkpoint, and evaluation contracts
tests/                 unit, CLI, optimizer, checkpoint, resume, and lineage tests
test_data/             portable episode/action contract fixture
third_party/           pinned optional-source references and license notes
```
...
[results/paper_results.json](results/paper_results.json) and validated by
`tools/validate_results.py`. These are reference results, not outputs generated
by the CPU smoke backend.
