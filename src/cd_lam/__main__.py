...
def _build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser("smoke", help="run a deterministic library smoke test")
    ...
    for command, help_text in (
        ...
        stage.add_argument(
            "--synthetic",
            action="store_true",
            help="run the explicit deterministic lightweight integration backend",
        )
    ...
    train_smoke = subparsers.add_parser(
        "train-smoke",
        help="run Stage1, Stage2, bridge, and Stage3 integration training in order",
    )
