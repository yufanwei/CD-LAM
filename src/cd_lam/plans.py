...
def build_stage_plan(
    ...
    *,
    ...
    selected_device = device or config.runtime.device
    if synthetic and re.fullmatch(r"cuda(?::[0-9]+)?", selected_device) is None:
        raise PlanError("synthetic integration requires CUDA")

    mode = "synthetic" if synthetic else "external"
    adapter_specification = None if synthetic else (adapter or stage_config.adapter)
    adapter_identity = f"cd_lam.synthetic_{selected_device}" if synthetic else None
