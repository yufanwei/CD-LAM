"""Deterministic lightweight training backends for integration smoke tests."""
...
SYNTHETIC_ADAPTER_PREFIX = "cd_lam.synthetic_"
...
def _require_synthetic(context: StageContext) -> None:
    if context.plan.mode != "synthetic" or not context.plan.device.startswith("cuda"):
        raise StageExecutionError("the built-in integration backend requires CUDA")
    if not torch.cuda.is_available():
        raise StageExecutionError(
            "CUDA integration was requested but CUDA is unavailable"
        )
    expected_identity = f"{SYNTHETIC_ADAPTER_PREFIX}{context.plan.device}"
    if context.plan.adapter_identity != expected_identity:
        raise StageExecutionError("synthetic adapter identity changed after planning")


def _device(context: StageContext) -> torch.device:
    return torch.device(context.plan.device)
...
def run_stage1(context: StageContext) -> StageResult:
    ...
    current = (torch.randn(batch, 1, 8, 8, generator=generator) * 0.1).to(device)
    labels = (torch.arange(batch) % 3).to(device)
    mask = torch.zeros(batch, 8, 8, device=device)
    mask[:, 2:6, 2:6] = 1.0
    motion = (labels.float() + 1.0).view(batch, 1, 1, 1) * mask[:, None] * 0.03
    future = current + motion

    model = _TinyLAM().to(device)
    head = SigLIPActionHead(LATENT_DIM, projection_dim=16, hidden_dim=32).to(device)
...
def _acwm_batch(
    ...
    current = (torch.randn(batch, 16, generator=generator) * 0.2).to(device)
    latent = (torch.randn(batch, LATENT_DIM, generator=generator) * 0.2).to(device)
    true_weight = (torch.randn(LATENT_DIM, 16, generator=generator) * 0.05).to(device)
    target = current + latent @ true_weight
    return current, latent, target


def run_stage2(context: StageContext) -> StageResult:
    """Run a tiny latent-conditioned world-model optimization."""
    ...
    model = _TinyACWM().to(_device(context))
...
def _bridge_batch(
    ...
    actions = torch.randn(batch, ACTION_DIM, generator=generator).to(device)
    true_weight = (torch.randn(ACTION_DIM, LATENT_DIM, generator=generator) * 0.15).to(
        device
    )
...
def run_bridge_training(context: StageContext) -> StageResult:
    ...
    bridge_mlp = build_bridge_mlp().to(_device(context))
...
def run_stage3(context: StageContext) -> StageResult:
    ...
    bridge = ActionToLatentBridge(bridge_source).to(device)

    batch = max(4, min(context.plan.batch_size, 16))
    current = (torch.randn(batch, 16, generator=generator) * 0.2).to(device)
    actions = (torch.randn(batch, ACTION_DIM, generator=generator) * 0.2).to(device)
