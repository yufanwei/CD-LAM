...
def _external_files(config: RuntimeConfig, stage: str) -> list[tuple[str, Path]]:
    ...
    per_stage = {
        "stage1": [
            (
                "Stage-1 trainer",
                acwm / "cdlam_integration" / "lam" / "train.py",
            ),
            (
                "Stage-1 data adapter",
                acwm / "cdlam_integration" / "lam" / "data.py",
            ),
            (
                "Stage-1 mask adapter",
                acwm / "cdlam_integration" / "lam" / "mask_adapter.py",
            ),
            (
                "Stage-1 transforms",
                acwm / "cdlam_integration" / "lam" / "transforms.py",
            ),
        ],
        "bridge": [
            (
                "bridge trainer",
                acwm / "cdlam_integration" / "bridge" / "train.py",
            ),
            (
                "bridge encoder",
                acwm / "cdlam_integration" / "bridge" / "encoder.py",
            ),
            (
                "bridge trunk adapter",
                acwm / "cdlam_integration" / "bridge" / "trunk.py",
            ),
            (
                "bridge decoder",
                acwm / "cdlam_integration" / "bridge" / "decoding.py",
            ),
            (
                "bridge preprocessing",
                acwm / "cdlam_integration" / "lam" / "preprocess.py",
            ),
        ],
        "stage2": [
            (
                "Stage-2 trainer",
                acwm / "cdlam_integration" / "world_model" / "train.py",
            ),
            (
                "LAM registry loader",
                acwm / "cdlam_integration" / "world_model" / "registry.py",
            ),
            (
                "LAM model loader",
                acwm / "cdlam_integration" / "world_model" / "model_loader.py",
            ),
        ...
        "stage3": [
            (
                "Stage-3 trainer",
                acwm / "cdlam_integration" / "stage3" / "train.py",
            ),
    ...
    if "stage2" in stages:
        modules.extend(
            [
                ...
                "cdlam_integration.world_model.train",
            ]
        )
    if "stage3" in stages:
...
def _bridge_commands(
    ...
    argv = [
        str(python),
        str(acwm / "cdlam_integration" / "bridge" / "train.py"),
        "--cache",
        str(cache),
        "--action-key",
        "action_22",
