...
def _parser() -> argparse.ArgumentParser:
    ...
    for name in (*STAGES, "pipeline"):
        ...
        stage_parser.add_argument(
            "--allow-gpu",
            action="store_true",
            help="acknowledge that this command launches GPU training",
        )
    return parser


def _require_gpu_acknowledgement(args: argparse.Namespace) -> None:
    if not args.dry_run and not args.allow_gpu:
        raise ConfigError(
            "GPU launch is blocked; pass --allow-gpu after `doctor` succeeds"
        )


def _run_one(config: RuntimeConfig, stage: str, args: argparse.Namespace) -> None:
    _require_gpu_acknowledgement(args)
