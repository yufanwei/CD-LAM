...
def validate_source(root: Path) -> Path:
    source = root / "lamwm_pipline" / "tools" / "train_wm_compat_real.py"
    ...
    if PATCHED_EXPRESSION not in text:
        ...
        raise RuntimeError(
            f"Stage-2 source is not contract-safe: {detail}. Apply "
            "third_party/patches/acwm_manifest_fps.patch to the pinned external tree."
        )
    if SUMMARY_EXPRESSION not in text or '"scope": args.scope' not in text:
        raise RuntimeError(
            ...
            "Apply third_party/patches/acwm_summary_contract.patch."
        )
    return source
