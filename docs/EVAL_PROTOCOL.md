...
weighting. CoWTracker is used for FDCE only, never for training. Both are
optional for core unit/smoke tests, but both (or protocol-compatible cached
outputs) are required for exact FDCE reproduction.
