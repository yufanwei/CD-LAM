...
pretrained ACWM overlay, and a separate robot-action posttrained overlay.
Immutable revision `2a02c07f4f5d1b731e0cf4e4fa1b9767ed592c1f` retains the
tensor-verified model blobs and passed the guarded 10-file inventory,
manifest, size, and LFS SHA-256 checks.
...
user-obtained data, and the supported CPython 3.10 / PyTorch 2.7.0+cu128
`.venv` on a compatible CUDA 12.8 GPU host.
...
- The compact LAM and pretrain entries form one compatible pair. The
...
Use the paper BibTeX entry in the repository [README](../README.md#citation).
No DOI or arXiv identifier is asserted until one is published by the authors.
