...
ignored `configs/runtime.json`, fill the model and data paths, install the
single pinned GPU environment and staged ACWM source, and validate every real
stage with:
...
`2a02c07f4f5d1b731e0cf4e4fa1b9767ed592c1f` publishes three main tensor-exact
2B research entries; its LAM/pretrain pair and separate posttrain entry
must not be treated as one direct lineage.
The source release provides typed planners, deterministic host-side integration
tests, pinned 2B GPU launch wrappers, and a manifest-checked integration overlay.
