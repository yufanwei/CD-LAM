...
The command uses the supported CPython 3.10 `.venv`. Bootstrap installs
`pyarrow`, PyAV, and HDF5 support together with the training runtime; `h5py` is
used when `session_name` is read from an EgoDex HDF5 metadata file.

Verify the installed data tools before reading a large clip:
